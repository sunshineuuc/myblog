...
<img src='what-is-binary/20191123175857.png' class="nofancybox"/>
...
**反码**通常是用来由原码求补码或者由补码求原码的过渡码。根据定义：`正数的反码与其原码相同；负数的反码是原码符号位不变，其他位按位取反`。
>$+1 = 00000001_{反码} $
$-1 = 11111110_{反码} $

反码是一种**过渡码**，将反码进行加法运算$1 - 1 = 1 + (-1) = 00000001 + 11111110 = 11111111$，结果不为零，也是不正确的，下面看下补码

**补码** 根据定义，`正数的补码与其原码相同；负数的补码是在其反码的末位加1`，即：
>$+1 = 00000001_{补码}$
$-1 = 11111111_{补码}$

使用补码相加$1 - 1 = 1 + (-1) = 00000001 + 11111111 =$ `1` 00000000。对于溢出的位，计算机会直接忽略掉，即00000001 + 11111111 = `00000000`。从结果可以看出使用补码进行运算逻辑更为严谨，也更为符合计算机。

---

##### 计算机中二进制移位运算 #####

和十进制数一样，四则运算同样也可以使用在二进制数中，只要注意逢2进位即可。下面，我们就来重点看一下二进制数所特有的运算。二进制数所特有的运算——`移位运算`。移位运算指的是将二进制数值的各数位进行左右移位（shift=移位）的运算。移位有左移（向高位方向）和右移（向低位方向）两种。在一次运算中，可以进行多个数位的移位操作。

无论程序中使用的是几进制，计算机内部都会将其转换成二进制数来处理，因此都能进行移位操作。但是左移跟右移不同，下面分开说明。

**左移**是向高位方向移动且空出来的低位要进行补0操作，移位操作使最高位或最低位溢出的数字，直接丢弃。左移一位即表示乘2操作。
>0 0 1 0 0 1 1 1$_{移位前} = 1 * 2^0 + 1 * 2^1 + 1 * 2^2 + 1 * 2^5 = 39$
>
>1 0 0 1 1 1 `0 0`$_{左移两位} = 1 * 2^2 + 1 * 2^3 + 1 * 2^4 + 1 * 2^7 = 156$

**右移**则向低位移动，空出来的高位根据标志位进行正数左补0，负数左补1的操作。右移一位即表示除2操作。
>0 0 1 0 0 1 1 1$_{移位前} = 1 * 2^0 + 1 * 2^1 + 1 * 2^2 + 1 * 2^5 = 39$
>
>0 0 1 0 0 1 . `1 1`$_{移位后} = 1 * 2^{-2} + 1 * 2^{-1} + 1 * 2^0 + 1 * 2^3 = 9.75$
>
>$1 0 1 0 0 1 1 1_{移位前} => 1 1 0 1 1 0 0 0_{反码} => 1 1 0 1 1 0 0 1_{补码} => -39$ 
>
>`11`$1 1 0 1 1 0 . 0 1_{移位后} => 1 0 0 0 1 0 0 1 . 1 0_{反码} => 1 0 0 0 1 0 0 1 . 1 1_{原码} => -9.75$

**注意:**负数的补码转为原码也是通过`取反加一`。
